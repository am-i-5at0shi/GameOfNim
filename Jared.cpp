...
class Jared{
    public:
        static void makeNim_sum_zero(vector<int>& values,int nimsum){
    	    for(int i=31;i>=0;i--){
        ...
        static int nimsum(vector<int>& values){
    	    int nimsum=0;
        ...
        static int can_finish(vector<int>& stone_piles){
            int index,empty=0;
        ...
        static void take_turn(vector<int>& stone_piles){
            srand(time(0));
